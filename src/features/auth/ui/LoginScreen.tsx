...
import { signInWithGoogle, signInWithGithub, signInWithEmail, signUpWithEmail } from '@/shared/lib/supabase'
  ...
  if (success) {
    return (
      <div className="fixed inset-0 bg-[#050508] flex items-center justify-center z-50 overflow-hidden">
        <AuthBackground />
        <div className="relative z-10 p-10 py-12 rounded-3xl bg-white/[0.03] backdrop-blur-2xl border border-white/[0.08] shadow-2xl max-w-[420px] w-full text-center space-y-6">
          <div className="w-16 h-16 bg-blue-500/20 rounded-full flex items-center justify-center mx-auto mb-4">
            <svg className="w-8 h-8 text-blue-400" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
            </svg>
          </div>
          <h2 className="text-2xl font-semibold text-white">¡Verifica tu correo!</h2>
          <p className="text-white/60 text-sm">
            Hemos enviado un enlace de confirmación a <span className="text-blue-400 font-medium">{email}</span>. 
            Por favor, revisa tu bandeja de entrada para activar tu cuenta.
          </p>
          <button 
            onClick={() => setSuccess(false)}
            className="text-white/40 hover:text-white/70 text-sm transition-colors pt-4"
          >
  ...
  return (
    <div className="fixed inset-0 bg-[#050508] flex items-center justify-center z-50 overflow-hidden">
      ...
      <div className={`relative z-10 transition-all duration-1000 ${mounted ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'}`}>
        <div className="relative flex flex-col items-center gap-6 max-w-[420px] w-full px-10 py-12 rounded-3xl
        ...
        ">
          ...
          <div className="text-center space-y-2">
            ...
            <p className="text-white/30 text-sm leading-relaxed">
              {mode === 'login' ? 'Ingresa tus credenciales para continuar' : 'Únete a la nueva era de la productividad'}
            </p>
          </div>
          ...
          <form onSubmit={handleEmailAuth} className="w-full flex flex-col gap-4">
            {mode === 'signup' && (
              <div className="space-y-1.5">
                <input
                  ...
                  className="w-full px-5 py-3 rounded-2xl bg-white/[0.04] border border-white/[0.1] text-white text-sm focus:outline-none focus:border-blue-500/50 transition-all"
                  required
                />
              </div>
            )}
            ...
            <button
              ...
              className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-blue-600 to-purple-600 text-white font-medium text-sm hover:brightness-110 transition-all active:scale-[0.98] disabled:opacity-50"
            >
              {loading === 'email' ? 'Procesando...' : (mode === 'login' ? 'Iniciar Sesión' : 'Registrarse')}
            </button>
          </form>
          ...
          <div className="w-full flex items-center gap-4 py-2">
            ...
            <span className="text-[10px] uppercase tracking-widest text-white/20">o continuar con</span>
            <div className="flex-1 h-px bg-white/5" />
          </div>

          {/* Botones Sociales */}
          <div className="grid grid-cols-2 gap-3 w-full">
            <SocialButton icon="google" onClick={handleGoogle} loading={loading === 'google'} label="Google" />
            <SocialButton icon="github" onClick={handleGithub} loading={loading === 'github'} label="GitHub" />
          </div>
...
function SocialButton({ icon, onClick, loading, label }: { icon: 'google' | 'github', onClick: () => void, loading: boolean, label: string }) {
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className="flex items-center justify-center gap-2.5 px-4 py-3 rounded-xl bg-white/[0.03] border border-white/[0.06] hover:bg-white/[0.08] transition-all disabled:opacity-40"
    >
      {loading ? (
