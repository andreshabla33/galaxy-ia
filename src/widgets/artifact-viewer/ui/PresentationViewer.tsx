...
function ImagePreloader({
  ...
  useEffect(() => {
    ...
    // Safety timeout: show presentation after 45s regardless of image status
    const safetyTimer = setTimeout(() => {
      if (!cancelled) {
        console.warn('[ImagePreloader] Safety timeout reached (45s), showing presentation without all images')
        cancelled = true
        onComplete()
      }
    }, 45000)
...
function SlideNumber({ index, total, muted }: { index: number; total: number; muted: string }) {
  return (
    <div className="absolute bottom-3 right-4 md:bottom-4 md:right-5 z-10 flex items-center gap-1.5">
...
function SlideRenderer({ slide: rawSlide, index, total, colors }: { slide: Slide; index: number; total: number; colors: ColorScheme }) {
  ...
  return (
    <div className="w-full aspect-video rounded-2xl border border-white/[0.08] flex flex-col relative overflow-hidden shadow-2xl shadow-black/40" style={slideStyle}>
      ...
      {/* ── CONTENT ── */}
      <div className="flex-1 flex flex-col justify-center px-6 py-5 md:px-12 md:py-10 relative z-[1]">
...
export function PresentationViewer({ contenido, titulo }: PresentationViewerProps) {
  ...
  const handleKeyDown = useCallback((e: KeyboardEvent) => {
    ...
    else if (e.key === 'ArrowRight') setCurrentSlide(prev => Math.min(slides.length - 1, prev + 1))
  }, [slides.length])
